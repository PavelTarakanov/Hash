...
int list_dump(list_t* list, char* const dump_file_name, FILE* const html_dump_address)
{
    ...
    if (snprintf(command, MAX_COMMAND_SIZE*sizeof(char), "%s%lld%s", "dot dump.txt -T png -o dump_", dump_number, ".png") == 0)
    ...
    fprintf(html_dump_address, "IMAGE:\n"
                              "<img src=dump_%lld.png width = 700px>\n\n\n", dump_number);
...
int list_verify(list_t* list)
{
    ...
    if (list->list_array[0].data != NULL)
